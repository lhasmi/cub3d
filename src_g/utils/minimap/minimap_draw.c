...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/24 20:51:01 by gbohm             #+#    #+#             */
/*   Updated: 2023/11/24 20:51:50 by gbohm            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	minimap_draw_tile(t_scene *scene, t_vec position)
{
	...
	center = get_center();
	player = vec_correct(vec_mult_scalar(scene->player.position, MAP_SCALE),
			1.0);
...
void	minimap_draw(t_scene *scene)
{
	...
	ix = 0;
	center = get_center();
	draw_square(scene->minimap, vec_create(0, 0, 0), 10 * MAP_SCALE,
		0x000000ff);
	...
	draw_square(scene->minimap, vec_sub(center, vec_create(3, 3, 0)), 7,
		0xff7800ff);
}
