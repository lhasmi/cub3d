...
/*   Updated: 2023/05/06 15:36:56 by lhasmi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_blocked_path(t_map *map)
{
	...
	while (i < map->height - 1)
	{
		...
		while (j < map->width - 1)
		{
			if ((map->tiles[i][j] == 'P' && map->tiles[i + 1][j] == 'E' && map->tiles[i + 2][j] == 'C') ||
				    (map->tiles[i][j] == 'C' && map->tiles[i + 1][j] == 'E' && map->tiles[i + 2][j] == 'P') ||
				    (map->tiles[i][j] == 'P' && map->tiles[i][j + 1] == 'E' && map->tiles[i][j + 2] == 'C') ||
				    (map->tiles[i][j] == 'C' && map->tiles[i][j + 1] == 'E' && map->tiles[i][j + 2] == 'P'))
				free_map(map, "Player is blocked by exit and can't reach the collectible");
            j++;
		}
        i++;
	}
